...
- **範圍界線**：這次只做後端，前端「執行監控」畫面仍是單裝置寫死的舊版面，尚未接上 `sessions[]`；多裝置並排/分頁顯示（使用者選定的方向）會先出 HTML mockup 給使用者確認過才動 React 代碼，之後另外記錄。
