...
**LuckyLink JP 監控永遠卡在「等待 Poller 啟動...」的根因與修復（2026-08-17，v4.6.1）**：`server/luckylink-poller.mjs`（Local Agent 端 spawn 的獨立 poller 腳本）從一開始就沒有被加進 `AGENT_SOURCE_WHITELIST`（`server/routes/machine-test.ts`）——這份清單決定 `install.bat` 與「更新 source files」會下載哪些檔案到使用者機器的 Agent 目錄，漏掉的檔案永遠不會出現在 Agent 端。`agent-runner.ts` 在 `autospin_start` 時 spawn poller 前有 `existsSync(pollerPath)` 檢查，但先前檔案不存在時只印一行 `console.warn` 到 Agent 本機終端機，完全不會透過 WS 回報給前端——所以無論重啟幾次 session、設定再正確，畫面永遠停在「等待 Poller 啟動...」，跟連線/重連完全無關（先前一度誤判成 stale session 問題，已被使用者實測重啟後依然故障推翻）。修法三點（跟 CodeX 討論定案）：
1. `AGENT_SOURCE_WHITELIST` 補上 `luckylink-poller.mjs`，之後新裝或按「Local Agent」頁面「更新 source files」按鈕（呼叫 `/api/local-agent/agent/:id/update-sources`，會重新下載整包 whitelist 到 `process.cwd()/server/...`）都能補齊，**不需要重跑 install.bat**；因為 poller 是每次 session 啟動時才從磁碟 spawn（不是 process 啟動時載入快取），理論上補檔後下一次啟動 AutoSpin 就會生效,不需要重啟 Local Agent process 本身
2. `agent-runner.ts` 新增 `sendLuckylinkError()` helper，在檔案缺失／poller spawn 失敗（`error` 事件）／poller 非正常結束（`close` 事件且 `code !== 0 && code !== null`，`null` 通常代表被 SIGTERM 正常中止不算異常）三種情況都送出 `luckylink_event`（`type: 'luckylink_error'`，帶 `code: 'LUCKYLINK_POLLER_MISSING'/'LUCKYLINK_POLLER_SPAWN_ERROR'/'LUCKYLINK_POLLER_EXIT'`），沿用既有的 `luckylink_event` WS→SSE 轉發路徑（`worker.ts` → `broadcastLuckylinkEvent()`），不用新開一條通道
3. **同步修正前端既有 bug**：`AutoSpinPage.tsx` 的 `luckylink_error` handler 先前寫成 `setLuckylinkStatus(prev => prev ? {...} : prev)`——只有先前已經收到過 `luckylink_start`（`luckylinkStatus` 非 null）才會套用錯誤訊息，poller 從未成功啟動過的情況下這個 guard 會讓錯誤事件被直接丟棄，UI 還是卡在「等待 Poller 啟動...」（因為畫面判斷式是 `!luckylinkStatus ? '等待 Poller 啟動...' : (...)`，null 狀態沒有顯示錯誤的管道）。改成 `luckylinkStatus` 為 null 時也初始化一份帶 `error` 的狀態物件，錯誤才會真正顯示出來（沿用既有的紅字錯誤框渲染邏輯，不用改 JSX）。

**多裝置並行支援（2026-08-18，後端 v4.8.0＋前端 v4.9.0，皆已完成）**：使用者要求「同一個帳號同時操控多台 Local Agent 裝置各自跑一批機台」，先前架構下一個帳號同時間只能有一個 AutoSpin session（綁一台裝置）——起因是全站共用的重任務鎖（`heavy-task-guard.ts` 的 `activeTasks`）純綁帳號 key，完全沒有裝置維度，這把鎖同時被 10 種其他功能共用（Jira 批次開單/評論/轉換狀態、Machine Test、OSM UAT、Game Show 統計/圖片比對、TestCase 生成×2）。跟 CodeX 討論後採用「不動共用鎖語意，另開明確 scoped API」的做法（B+ 精神延續自機種設定檔複合鍵那次的決策風格）：
- `heavy-task-guard.ts` 新增 `tryStartScopedHeavyTask(req, type, label, scopeKey)`，內部委派同一個私有函式，`lockKey = scopeKey ? \`${userKey}::${scopeKey}\` : userKey`，`activeTasks` 這個 Map 的 key 改用 `lockKey`；`HeavyTaskToken` 新增 `lockKey` 欄位（無 scope 時等於 `userKey`），`finishHeavyTask()` 改用 `token.lockKey` 釋放。`tryStartHeavyTask()` 簽章/呼叫點完全不動（內部改呼叫同一支私有函式、scope 傳 `undefined`），其餘 10 個既有呼叫點行為零改變。AutoSpin 呼叫時傳 `scopeKey = \`agent:${agentId}\`。
- **`heavy_tasks` 表新增 `lock_key` 欄位**：unscoped 任務這欄等於 `user_key`（向下相容），`user_key` 欄位本身完全不動（歷史查詢如 `getRecentHeavyTasksForRequest()` 不受影響）；伺服器重啟復原邏輯改用 `row.lock_key || row.user_key` 當 Map key，否則 scoped 鎖重啟後會被摺疊回裸帳號 key，多裝置語意在重啟後就消失。
- **agentId 一路貫穿**：`hub-dispatch` 送 WS `autospin_start` 訊息時帶上 `agentId: agent.agentId`；`agent-runner.ts` 組 Python 的 `uriArg`（`toppath-agent://?server=...&user=...`）多加 `&agent=<agentId>`；`toppath-agent.py` 的 `agent_id` 補成跟 `user_label`/`session_id` 同一套「module-level global，parent 讀一次→經 `multiprocessing.Process` args 傳給每個 child→child 內用 `global` 賦值」模式（Windows/macOS `spawn` 模式下 child 重新 import 整份模組，不會自動繼承 parent 的全域變數），`poll_stop()` 斷線重連註冊也帶上 `agentId`，否則重連會退回沒有 scope 的舊語意。
- **`/agent/start` 的兩處迴圈都補上 agentId 比對**：「reconnect 加入既有 session」的比對條件、以及「Stop existing agent sessions for this user」清理迴圈，都從只比對 `userLabel` 改成同時比對 `agentId`——否則裝置 B 重新派工時，清理迴圈會把裝置 A 正在跑的 session 一起停掉，直接違背多裝置並行的目的（這是最容易漏掉的一步，第一版設計只鬆了鎖、沒改這段清理邏輯，是 CodeX review 抓到的）。
- **`hub-stop` 的雙保險迴圈**（WS 停止指令之外，額外標記 Python 端 `stopRequested` 的迴圈）同樣補上 `agentId` 比對；`stop-all`（前端「停止」按鈕，語意就是「這個帳號全部停」）刻意維持不分裝置不改，跟 `hub-stop` 是兩個不同語意的按鈕。
- **`GET /agent/status` 改回傳 `sessions[]` 陣列**（每筆含 `sessionId`/`startedAt`/`agentId`/`hostname`），向下相容保留 `sessionId`/`startedAt`/`running` 對應陣列第一筆，現有前端行為零改變；前端要接上多裝置畫面時改讀 `sessions[]`。
- **`AgentSession` 新增 `agentId` 欄位**：`autospin_agent_sessions` 表的 5 秒快照持久化本來就是整包物件序列化，新增欄位不用改寫入邏輯；復原時對舊快照（欄位不存在）fallback 空字串。
- **前端已於 v4.9.0 完成**：mockup 經 CodeX 兩輪 review（響應式斷點、tab 窄版縮短、多層 scroll 邊界提示、非主視角錯誤更醒目、停止按鈕 confirm 帶 hostname）與使用者確認方向後才動 React。實作細節見下方獨立條目。

**多裝置並行前端實作（2026-08-18，v4.9.0）**：`AutoSpinPage.tsx` 的「執行監控」遠端 Agent 模式從單裝置畫面改成頂部裝置釘選列＋下方 grid，架構跟 CodeX 討論定案：
- **新增 `src/pages/useAgentSession.ts`**：單一 session 的 SSE 連線／日誌／截圖輪詢／LuckyLink 狀態／暫停/繼續/Spin間隔，全部集中在 `useAgentSession(sessionId, agentId, userLabel)` 這支 hook 裡。**identity 以 `sessionId` 為主，`agentId` 只用來組 `hub-stop` 的請求 body**（那支端點是「命令哪個裝置的 Local Agent」，不是 session 操作）——不能拿 `agentId` 當這個 hook 的 key，否則同一台裝置停掉又開新 session 時，舊 session 的 log/截圖/LuckyLink 狀態會殘留跨到新 session 上（CodeX review 特別點名的風險）。`sessionId` 變更時完整重置所有 state 並重新開一條 SSE；unmount／`sessionId` 變成 null 時完整 cleanup：`EventSource.close()`、reconnect timer（`setTimeout`）、screenshot polling interval 三者都要清，避免多個面板實例互相殘留計時器。
- **新增 `src/pages/AutoSpinDeviceSessionPanel.tsx`**：純消費 `useAgentSession` 回傳值畫 UI，不自己碰 SSE/timer。依 `isPrimary` prop 切換兩種渲染：
  - **主視角（完整三層）**：狀態列（hostname/session/uptime/暫停繼續/停止此裝置）→ Spin 間隔調整 → 機台清單+日誌（篩選/搜尋/pinus分類/下載/清空，跟原本單裝置版一致）+ 右側 LuckyLink/SLS/截圖監控**改成 tabs 切換**（原本是三個各自可收合的堆疊面板，多裝置後資訊密度太高，改 tabs 一次只看一個，跟 mockup 一致）
  - **精簡卡片（非主視角）**：hostname＋狀態點＋機台數/uptime/錯誤數摘要＋日誌尾巴 4 行，點擊觸發 `onPromote` 切換為主視角；**有錯誤時左側 3px 紅邊＋淡紅底＋脈動紅點**（CodeX review 要求「不能只靠文字顏色，容易被忽略」），不是只在文字顏色上做區分
  - **停止此裝置按鈕**：`confirm()` 對話框明確帶 hostname（例如「確定要停止 DESKTOP-QA01 嗎？」），避免多裝置同時顯示時誤停錯台（mockup 階段就先標記的實作要求，這次落地）
- **`AutoSpinPage.tsx` 只保留全域狀態**：`agentSessions`（`/agent/status` 回傳的 `sessions[]`）、`pinnedAgentIds`、`primaryAgentId`——不再塞任何單一 session 的 log/截圖/LuckyLink 資料，這些完全交給每個 `DeviceSessionPanel` 實例自己的 hook 管理。全域輪詢（`fetchStatus()`，4 秒一次）偵測到新的 `agentId` 出現時自動釘選（用 `seenAgentIdsRef` 追蹤「曾經看過的 agentId」，避免使用者手動取消釘選後又被輪詢自動加回來）；`primaryAgentId` 若目前值已不在釘選清單裡，自動 fallback 成清單第一筆。
- **裝置卡片版面**（對應 mockup 的兩種情境）：釘選 1~2 台，`autospin-device-grid--cols-1`/`--cols-2` 並排各自完整版面；3 台以上，1 台（`primaryAgentId` 或預設第一筆）用 `--cols-1` 滿版完整顯示，其餘用 `autospin-condensed-row`（2 欄 grid）渲染精簡卡片。CSS 補在 `App.css`：`< 1100px` 兩欄 grid 改單欄堆疊，卡片本身 `< 640px` 左右兩欄改上下堆疊，都是 mockup 階段 CodeX review 要求的 responsive breakpoint 原封不動搬進正式 CSS。
- **`handleDispatchAgent`/`handleStopHub` 語意修正**：派工按鈕的 disabled 條件從「`agentRunning`（任一裝置在跑就擋）」改成「選定的這台裝置本身忙碌或已在跑」——原本的條件在多裝置情境下是錯的，會導致明明有其他裝置閒置，卻因為別的裝置正在跑而擋住新派工。`handleStopHub` 拿掉了 `agent/stop-all` 呼叫（那支是整帳號全停，多裝置下會誤停其他裝置），只呼叫 `hub-stop({agentId: selectedAgentId})`，精準只停選定的那一台。
- **伺服器端 (fallback) 模式完全不受影響**：`connectSSE`/`logs`/`captures`/`handleStart`/`handleStop` 等既有邏輯原封不動保留，只是把先前跟 hub 模式共用的 `isAgent` 參數拿掉（hub 模式的 SSE 已經完全移到 hook 內，不再需要這個分支）。
- **已知限制**：這次開發環境的瀏覽器自動化工具（`browse` skill）在本機遇到 `stdio must be an array` 的相容性問題無法使用，僅能透過 `tsc -b`（全專案含 project references）與完整 `npm run build` 驗證型別/建置正確性，**沒有實際在瀏覽器裡點過這個功能**——部署後第一次使用建議請使用者實測，若有 runtime 才會出現的問題（例如 CSS 排版細節、真實多裝置情境下的邊界情況）需要之後再修。

**Local Agent WebSocket 殭屍連線問題（2026-08-18，v4.9.3）**：部署後正式環境真實案例——使用者的 Local Agent 終端機從連線成功後（`Connected — ready`）就再也沒有任何後續訊息，畫面上的裝置釘選列完全偵測不到這台裝置（`hub-agents` 持續回傳空清單），但 Local Agent 自己完全沒有印出任何 `Disconnected` 訊息，只能手動重啟才會恢復。根因：`agent-runner.ts` 的重連邏輯（`ws.on('close')` → 5秒後 `connect()`）完全是**被動**的，只在收到 WebSocket 的 `close` 事件時才會觸發；但當伺服器 process 重啟時（部署很常見，今天連續推了 5 次），中間隔著 Nginx 反代（`worker.ts` 本來就有「每 30 秒 `ws.ping()` 防止 Nginx `proxy_read_timeout` 閒置斷線」的機制，side-confirm 了 Spug 後面確實接 Nginx），client 端的連線物件可能變成「殭屍連線」——TCP/WS 連線物件還在，但 `close` 事件遲遲不觸發甚至永遠不觸發，Local Agent 就會卡在「顯示已連線、但伺服器早就不認得它」的狀態。修法（跟 CodeX 討論定案）：`agent-runner.ts` 補上 client 端**主動**心跳存活檢查——每 20 秒 `ws.ping()`，若超過 40 秒沒收到任何 `pong`（`ws.on('pong', ...)` 更新 `lastPongAt`），視為死連線、主動 `ws.terminate()`（不是 `close()`——`terminate()` 強制立即摧毀 socket，可靠觸發既有的 `close` handler，不用等一個可能永遠不會來的優雅關閉握手），沿用既有的 5 秒後重連邏輯，不需要另外呼叫 `connect()`（避免雙重重連）；`terminate()` 前印出明確的 `Heartbeat timeout, terminating stale connection` log，方便之後使用者截圖時能直接判斷是不是同類問題。`heartbeatTimer` 在 `close`／`error` 兩個事件都會 `clearInterval()`，避免舊連線的計時器殘留。**這個檔案是透過 `AGENT_SOURCE_WHITELIST` 分發的，既有使用者要到「Local Agent」頁面點「更新 source files」（或重新下載 install.bat）才會拿到這個修法，光是 server 端部署新版不會自動更新到使用者本機已經在跑的 Local Agent 程式碼**。

**Migration 硬化教訓（同一批，2026-08-18）**：`lock_key` 這個 migration 開發過程中真的把本機 pm2 環境的 server 搞掛過一次——`CREATE INDEX` 一度被放進最上面 `CREATE TABLE IF NOT EXISTS` 那個 db.exec() 區塊，對既有資料庫（表已存在、只是缺這個新欄位）會直接對不存在的欄位建索引、拋 `SqliteError`，整個模組載入死掉、server 開機即崩潰；且崩潰後底層 node process 沒乾淨釋放 port 3000，導致 pm2 之後的自動重啟全部撞到 port 衝突陷入 restart loop（有真實使用者當下在用，造成一段時間無法連線）。跟 CodeX 討論後定案的教訓與修法：**schema migration 失敗不能吞錯讓 server 帶著半套 schema 繼續跑**（之後每次操作都可能踩到不一致狀態卻沒人發現，比直接讓 server 開機失敗更難排查）——改成明確分步驟＋每步驗證（`ALTER TABLE` 後用 `PRAGMA table_info` 確認欄位真的存在、`CREATE INDEX` 後用 `sqlite_master` 確認索引真的建立）＋失敗就印出詳細狀態後 `process.exit(1)` fail fast。**這次刻意只硬化這個新 migration，沒有回頭改動檔案裡其他既有的 `ALTER TABLE` 遷移**——那些數量不少，屬於更大範圍的技術債，CodeX 建議另開任務處理，不塞進這次的 commit。

**`kill_timeout` 加了又拿掉的教訓（同一天稍晚，2026-08-18）**：上面那次事故發生時，`ecosystem.config.cjs` 曾補上 `kill_timeout: 5000`（PM2 預設 1600ms），想法是給舊 process 更多時間乾淨關閉、降低「crash 後殘留 process 佔 port」的 blast radius。**部署後使用者實測回報：正式環境每次 Spug 重新部署後，網站第一次載入會卡住（最長觀察到 30-60 秒），重新整理才恢復正常，且明確是「以前不會這樣」**——時間點精準對上 `kill_timeout` 調整那次部署。跟 CodeX 討論後判斷：這次事故的根因是「migration 讓新 process 起不來」，不是「舊 process 沒時間關」，延長 shutdown window 本來就擋不住這種問題，反而讓部署期間的空窗期變長、使用者更容易撞到；migration 本身已經改成分步驟驗證＋fail fast（見上一條），這層防禦性設定的邊際價值不高、卻已證實有真實 UX 成本。**改法：移除 `kill_timeout`，回 PM2 預設**，不抓折衷值。（30-60 秒這個量級明顯超過 5 秒與 1.6 秒的差距，不完全是 `kill_timeout` 一個因素造成——如果拿掉後首次載入還是慢，下一步要查的方向是 PM2 restart 流程本身／health check／反向代理逾時設定，不是回頭再調這個數字。）
