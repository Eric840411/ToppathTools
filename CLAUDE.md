...
### 功能說明
側邊欄帳號區塊顯示一個小稱號徽章，依帳號累計操作次數自動晉升，靈感取自《凡人修仙傳》的修煉境界（練氣期→築基期→金丹期→元嬰期→化神期→煉虛期→合體期→大乘期→渡劫期，共 9 階，門檻可調整）。純展示用途，不影響任何權限判斷。

**計數來源**：`server/shared.ts` 的 `addHistory()`（全站唯一的歷史紀錄寫入點）每次呼叫時，除了寫入 `operation_history`（這張表每 7 天會被自動清空，不能拿來算累計）之外，另外對 `account_cultivation` 表做 upsert 遞增，這張表永不清除。`GET /api/account/cultivation` 回傳目前境界、累計次數、下一階名稱與門檻。

### 使用者操作
| 操作 | 說明 |
|------|------|
| 查看目前境界 | 側邊欄帳號名稱下方的小徽章，滑鼠移上去顯示累計次數與距離下一階還差幾次 |
