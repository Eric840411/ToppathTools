...
| 類別 | 筆數 | 特徵 | 該怎麼辦 |
|---|---:|---|---|
| **A. 只寫 note、從不失敗** | 12 | verifier 只 `notes.push('⚠️…')`，沒有 `criticalFails` | **現在是綠燈但什麼都沒驗**。改成真斷言或標 manual 都是行為變更，要獨立版號 |
...
**A 類是目前最該處理的**，因為那 12 筆現在跑起來是通過的、但一個斷言都沒執行——跟 v4.38.0 掃描器要抓的是同一件事，只是它們走的是舊路徑（`SUBTYPE_MAP`），那條路上沒有零斷言守門。
