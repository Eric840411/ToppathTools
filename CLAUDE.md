...
**已知限制**：`poll_stop()` 內建的「session 遺失後自動重新登錄」fallback（例如伺服器重啟）目前是每個 child 各自獨立偵測、各自獨立重連，沒有跨 process 協調——理論上多個 child 同時偵測到同一次伺服器重啟時，可能會各自搶著重新註冊、互相覆蓋彼此的新 session（原本單一 process 架構下這是不會發生的邊界案例）。這是既有 fallback 機制、不是主要流程，暫不特別處理，之後真的遇到問題再回來看。
