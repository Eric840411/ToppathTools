...
const AGENT_SOURCE_WHITELIST: Record<string, string> = {
  ...
  'uat-runner/ui-popup.js':            join(SERVER_ROOT, 'uat-runner', 'ui-popup.js'),
...
router.get('/api/machine-test/agent/install.bat', (req, res) => {
  ...
  const bat = [
    ...
    '',
    'REM Replace start.bat before lengthy dependency installation',
    '(',
    '  echo @echo off',
    '  echo cd /d %%~dp0',
    ...
    ') > "%AGENT_DIR%\\start.bat.new"',
    'move /Y "%AGENT_DIR%\\start.bat.new" "%AGENT_DIR%\\start.bat" >nul',
    'if %errorlevel% neq 0 (',
    '  echo [ERROR] Failed to replace start.bat. Close any program using it and retry.',
    '  pause',
    '  exit /b 1',
    ')',
    'echo [OK] Updated start.bat with the latest agent token',
    '',
    'REM npm install',
    'cd /d "%AGENT_DIR%"',
    ...
    '',
    'REM AutoSpin Python engine deps (best-effort, does not block install — AutoSpin is optional)',
    'set PYDEPS_OK=0',
    'where pip >nul 2>&1',
    'if errorlevel 1 goto :pydeps_missing',
    '',
    ...
    'echo.',
    'echo ***************************************************************',
    'echo   [WARNING] AutoSpin Python dependencies are NOT ready yet!',
    'echo   AutoSpin will fail to start until you run this manually:',
    'echo.',
    'echo     pip install opencv-python numpy requests playwright',
    'echo     python -m playwright install chromium',
    'echo.',
    'echo   (If "pip" is not found: install Python 3 from python.org,',
    'echo    check "Add python.exe to PATH" during setup, then re-run',
    'echo    this installer or the two lines above.)',
    'echo ***************************************************************',
    'echo.',
    '',
    ':pydeps_done',
    ...
    'if "%PYDEPS_OK%"=="0" echo.',
    'if "%PYDEPS_OK%"=="0" echo  [!] AutoSpin Python deps were NOT installed - see WARNING above.',
    'echo ===================================================',
    'echo.',
    'pause',
  ].join('\r\n')
