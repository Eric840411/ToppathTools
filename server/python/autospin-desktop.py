...
# EDGEDRIVER_EXE removed: using Playwright Chromium instead
...
# =========================== Playwright 相容層（取代 Selenium API）===========================

class _By:
    # Selenium By 相容 stub（供現有程式碼無縫使用）
    CSS_SELECTOR = "css"
    XPATH = "xpath"
    ID = "id"
    CLASS_NAME = "class"
...
class TimeoutException(Exception):
    # Selenium TimeoutException 相容
    pass


class NoSuchElementException(Exception):
    # Selenium NoSuchElementException 相容
    pass


def _to_pw_selector(by, selector: str) -> str:
    # 將 (By, selector) 轉換為 Playwright selector 字串
...
class _ElementAdapter:
    # 將 Playwright ElementHandle 包裝成 Selenium WebElement 相容介面
...
class _PlaywrightDriver:
    # Playwright Page 的 Selenium WebDriver 相容包裝器。
    # 所有 self.driver.xxx 呼叫均透過此類轉發至 Playwright API。
    ...
    def find_elements(self, by, selector: str):
        pl_sel = _to_pw_selector(by, selector)
        handles = self._page.query_selector_all(pl_sel)
        return [_ElementAdapter(h) for h in handles]

    def find_element(self, by, selector: str):
        pl_sel = _to_pw_selector(by, selector)
        h = self._page.query_selector(pl_sel)
        if h is None:
            raise NoSuchElementException(f"No element found: {selector}")
        return _ElementAdapter(h)

    # ── JS 執行 ────────────────────────────────────────────────────────
    def execute_script(self, script: str, *args):
        # 執行 JS（同步）。若傳入 _ElementAdapter，自動解包為 ElementHandle。
        pw_args = [a._h if isinstance(a, _ElementAdapter) else a for a in args]
        if pw_args:
            elem = pw_args[0]
            if "scrollIntoView" in script:
                elem.scroll_into_view_if_needed()
                return None
            elif "arguments[0].click()" in script:
                elem.click()
                return None
            else:
                fn = f"(el) => {{ {script} }}"
                return self._page.evaluate(fn, elem)
        else:
            fn = f"() => {{ {script} }}"
...
class WebDriverWait:
    # Selenium WebDriverWait 相容（以 Playwright 實作）
    ...
    def until(self, condition):
        return condition(self._driver, self._timeout)


class _EC:
    # Selenium expected_conditions 相容

    @staticmethod
    def presence_of_element_located(locator):
        by, selector = locator
        def _cond(driver, timeout):
            return wait_for(driver, by, selector, timeout=timeout)
        return _cond
    ...
    def presence_of_all_elements_located(locator):
        by, selector = locator
        def _cond(driver, timeout):
            return wait_for_all(driver, by, selector, timeout=timeout)
        return _cond
    ...
    def element_to_be_clickable(locator):
        ...
        def _cond(driver, timeout):
            pl_sel = _to_pw_selector(by, selector)
            try:
                driver._page.wait_for_selector(pl_sel, state='visible', timeout=timeout * 1000)
                h = driver._page.query_selector(pl_sel)
                if h is None:
                    raise TimeoutException(f"Element not found: {selector}")
                return _ElementAdapter(h)
            except PlaywrightTimeoutError:
                raise TimeoutException(f"Timeout waiting for clickable: {selector}")
        return _cond
...
def wait_for(driver, by, selector, timeout: float = 8.0):
    # 等待單一元素存在（presence），回傳元素；逾時拋 TimeoutException
    try:
        pl_sel = _to_pw_selector(by, selector)
        driver._page.wait_for_selector(pl_sel, timeout=timeout * 1000)
        h = driver._page.query_selector(pl_sel)
        if h is None:
            raise TimeoutException(f"Element not found after wait: {selector}")
        return _ElementAdapter(h)
    except PlaywrightTimeoutError:
        raise TimeoutException(f"Timeout ({timeout}s) waiting for: {selector}")


def wait_for_all(driver, by, selector, timeout: float = 8.0):
    # 等待多個元素存在（presence），回傳 _ElementAdapter 清單；逾時拋 TimeoutException
    try:
        pl_sel = _to_pw_selector(by, selector)
        driver._page.wait_for_selector(pl_sel, timeout=timeout * 1000)
        handles = driver._page.query_selector_all(pl_sel)
        return [_ElementAdapter(h) for h in handles]
    except PlaywrightTimeoutError:
        raise TimeoutException(f"Timeout ({timeout}s) waiting for all: {selector}")


def safe_click(driver, elem) -> bool:
    # 通用點擊（Playwright 版本）：滾動到視窗中，再點擊；失敗不拋例外而回傳 False
    try:
        h = elem._h if isinstance(elem, _ElementAdapter) else elem
        ...
        try:
            h = elem._h if isinstance(elem, _ElementAdapter) else elem
            driver._page.evaluate("el => el.click()", h)
            return True
        except Exception:
            return False
...
def is_404_page(driver):
    # Playwright version: detect 404 pages via title, content, and URL
    try:
...
class _HistoryMonitorLegacy:
    ...
    def _fetch_records(self, driver) -> List[dict]:
        # Playwright version: call pinus.request via Promise-based page.evaluate
        try:
            # 先嘗試取得 uid（從頁面 JS 環境）
            uid = str(driver._page.evaluate(
                "() => (window._uid || (window.pinus && window.pinus.uid) || '')"
            ) or "").strip()
            ...
            # 使用 Promise 封裝 pinus.request（取代 Selenium execute_async_script）
            pinus_js = (
                "([monitorUid]) => new Promise((resolve) => {"
                "var p = window.pinus;"
            ...
            result = driver._page.evaluate(pinus_js, [uid])
...
class GameRunner:
    ...
    def _build_driver(self):
        # Build Playwright Chromium browser (replaces Edge WebDriver)
        # Uses mobile User-Agent and 432x859 viewport matching original Edge setup
        try:
            pw = sync_playwright().start()
            browser = pw.chromium.launch(
                headless=False,
                args=["--window-size=432,859"],
            )
            context = browser.new_context(
                viewport={"width": 432, "height": 859},
                user_agent=(
            ...
            return _PlaywrightDriver(page, browser, pw)
        except Exception as e:
