...
**同一支函式的第二個洞（CodeX 指出）**：按鈕序號用 `td.querySelectorAll(tag)` 算「這格裡第幾個後代」，卻產出 `nth-of-type`——那是「在自己父層裡同 tag 第幾個」。兩顆按鈕各自包在自己的 `<span>` 裡時，兩顆都是 `nth-of-type(1)`，選擇器會一次命中兩顆。改用 Playwright 的 `:nth-match(<sel>, N)`。
