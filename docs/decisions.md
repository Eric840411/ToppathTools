...
## [2026-09-16] 「沒驗到」要驗證最終分類，不能只驗警告有沒有出現

**決策**：斷言「某個檢查沒有被執行時不該算通過」時，測試一律驗**最終分類**
（這筆結果會被算進 `passCount` 還是 `skipCount`），不能只驗 notes 裡有沒有警告字樣。

**為什麼**（2026-09-16 實際踩到，v4.148.0 → v4.148.1）：

`assert_sorted` 在欄名打錯時會印「✅ 0 列排序正確」並讓整筆 TC 通過。
修的時候我把「沒驗到」改成 `warn`，並補了一條測試檢查 notes 沒有印「排序正確」。

**兩件事都是錯的**：

1. `warn` 在這套引擎裡**不影響 pass 判定**，而 runner 的統計是
   `if (pass && manual) skipCount++; else if (pass) passCount++`
   ——`warn` 是 `pass=true, manual=false`，直接落進 `passCount`。
   **修正等於白做，而報表照樣是綠的。**
2. 那條測試在 `warn` 之下**照樣綠**——它防不住我剛加的那個洞。

正確的預設是 `manual`：runner 會走 `skipCount` 並把 Lark 回填成「需人工」，
語意才對得上「機器判不了」。

**適用範圍**：所有「查了但沒查到東西」的路徑——空表格、整欄型別不符、
部分列無法解析、找不到檔案。共同判準是**「沒驗到」跟「驗過了」不可以長得一樣**。

⚠️ **順帶一條**：不可以 `filter` 掉比不了的列再拿剩下的下結論。
實測 `[900, 'oops', 100]` → 只留 `[900, 100]` → 「✅ 2 列排序正確」→ 通過，
中間那列的違規被靜默跳過；而「2 列」跟表格有 3 列這件事不會有人去對。
**默默縮小驗證範圍跟直接不驗一樣危險。**

⚠️ **測試不可以複製 runner 的規則**（CodeX review）：`countsAsPass` 一開始是在
測試裡另寫一份判斷，兩邊日後只要有一邊改了，測試會繼續綠、實際統計卻已經不同。
規則收在 `block-engine.js` 的 `countBucket()`，runner 與測試 import 同一支。

**重看條件**：`countBucket` 目前把 `pass=false, manual=true`（blocked）算成 `fail`，
而 Lark 回填那邊用的是 `manual`——兩者不一致。這是既有行為，測試已經釘住；
要改的話會動到報表數字，必須獨立一版、獨立驗證。

---

## [2026-09-16] 錄製腳本改成團隊共用：A／B 分期與剩餘風險

**決策**：把 `uat_recorded_scripts` 從「依帳號隔離」改成「團隊共用」，
並且**只做 A，B 排下一版**。

| | 內容 | 這一版 |
|---|---|---|
| **A** | 共用讀寫、`revision` 樂觀鎖、同腳本執行互斥、`executedBy`／`updatedBy`、軟刪除 | ✅ v4.155.0 |
| **B** | `base/table/record` 整批回寫鎖，所有回寫入口共用同一把 | ⏳ 下一版 |

**為什麼可以分期**：B 要修的是一個**既有**缺口——重任務鎖是依操作者
（`taskUser(req).key`），所以**在共用之前**，兩個帳號各自的腳本綁到同一批
Lark TC、同時跑，就已經沒有任何東西擋。

⚠️ **但不能因此說「A 沒有改變風險等級」**（CodeX 更正過我這句）：
後果相同，但共用讓同一批 TC 更容易被多人同時執行，**機率提高了就是風險改變**。

⚠️ **剩餘風險（寫在這裡是為了下次有人問時不用重新推導）**：
A 之後仍然可能發生跨腳本／跨入口的撞寫。**同腳本互斥只是降低機率，不是解決那個洞。**

### 幾個做錯了不會報錯的點

| 決定 | 為什麼 |
|---|---|
| 樂觀鎖用**整數 `revision`**，不用 `updated_at` | 同一毫秒內兩次存檔會碰撞（CodeX）|
| 檢查與遞增在**同一句 SQL** | 先查再寫的話兩個請求會查到同一個版本，等於沒鎖 |
| 沒帶 `expectedRevision` 一律當衝突 | 放行就退回「後存的贏」，那正是要修的行為 |
| 衝突時**保留本地草稿**，不叫前端重新載入 | 重新載入會把使用者正在編輯的內容洗掉——跟被別人覆蓋的感受一樣 |
| 存檔成功也**不用回傳的 script 蓋畫面**，只更新 revision | 同上 |
| 執行鎖在**收尾時釋放**，不看 agent 連線 | **斷線不等於已經停止**：可能還在對方機器上跑、還在回寫 Lark |
| `owner` **仍參與刪除授權** | 它不是只剩顯示用（CodeX 特別更正）|
| 刪除要有**管理員**這條路 | 只讓建立者刪的話，建立者離職就沒人刪得掉 |
| 軟刪除、保留歷史結果 | 執行紀錄是稽核資料，不該跟著腳本一起消失 |
| 結果讀取跟腳本一致（登入即可）| 否則「一起維護」的人看不到彼此跑出來的結果，等於各做各的 |

### ⚠️ 又一次「測試複製了產品的規則」

樂觀鎖那組測試第一版是在測試裡**自己寫一段一模一樣的 UPDATE**。
把產品程式碼的 `revision` 條件拿掉之後，**測試照樣全綠**——它驗的是自己那份 SQL。

修法跟 `countBucket()` 那次一樣：把規則抽成 `saveRecordedScriptDoc()`，
產品與測試 import 同一支。抽完再注入同一個違規，就正確轉紅 3 項。

**這是同一個形狀第二次出現**（前一次見上面 2026-09-16 那條）。
判準：**測試裡只要出現跟產品相同的 SQL／判斷式，就是在驗自己。**
