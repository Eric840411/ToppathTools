  ...
  const AutoPanel = ({ platform }: { platform: AutoPlatform }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ ...panelStyle, display: 'flex', alignItems: 'center', gap: 16, padding: 12 }}><span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#334155', fontWeight: 600 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: apiReady ? '#16a34a' : '#94a3b8' }} />Node.js / Playwright</span><a href="/api/frontend-auto/setup/install.bat" style={downloadStyle}>install.bat</a><a href="/api/frontend-auto/setup/install.sh" style={downloadStyle}>install.sh</a></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 3fr) minmax(320px, 2fr)', gap: 16 }}>
          <section style={panelStyle}><div style={panelHeadStyle}><h3 style={h3Style}>Script List</h3><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search" style={{ ...inputStyle, width: 180 }} /></div><div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>{(['all', 'mine', 'public'] as const).map(item => <button key={item} onClick={() => setFilter(item)} style={filter === item ? activePillStyle : pillStyle}>{item === 'all' ? 'All' : item === 'mine' ? 'Mine' : 'Public'}</button>)}</div><table style={tableStyle}><thead><tr><th style={thStyle}>Name</th><th style={thStyle}>Steps count</th><th style={thStyle}>Last result</th><th style={thStyle}>Actions</th></tr></thead><tbody>{visible.map(script => <tr key={script.id} onClick={() => setSelectedScriptIds(prev => ({ ...prev, [platform]: script.id }))} style={{ background: selectedScriptIds[platform] === script.id ? '#eff6ff' : '#fff', cursor: 'pointer' }}><td style={tdStyle}>{script.name}</td><td style={tdStyle}>{parseAutoSteps(script.steps).length}</td><td style={tdStyle}>{latest.get(script.id) ?? 'unknown'}</td><td style={tdStyle}><button style={smallBtnStyle} onClick={e => { e.stopPropagation(); setSelectedScriptIds(prev => ({ ...prev, [platform]: script.id })); void runAutoScript(platform) }}>Run</button><button style={dangerBtnStyle} onClick={e => { e.stopPropagation(); void deleteAutoScript(platform, script) }}>Delete</button></td></tr>)}</tbody></table><button style={{ ...btnStyle, background: '#1d4ed8', marginTop: 12 }} onClick={() => setNewScriptOpen(prev => ({ ...prev, [platform]: !prev[platform] }))}>New Script</button>{newScriptOpen[platform] && <div style={{ display: 'grid', gap: 8, marginTop: 12 }}><input value={newScriptDraft[platform].name} onChange={e => setNewScriptDraft(prev => ({ ...prev, [platform]: { ...prev[platform], name: e.target.value } }))} placeholder="Script name" style={inputStyle} /><textarea value={newScriptDraft[platform].steps} onChange={e => setNewScriptDraft(prev => ({ ...prev, [platform]: { ...prev[platform], steps: e.target.value } }))} rows={6} style={{ ...inputStyle, fontFamily: 'Consolas, Monaco, monospace' }} /><button style={{ ...btnStyle, background: '#16a34a', width: 110 }} onClick={() => void createAutoScript(platform)}>Save</button></div>}</section>
          <section style={panelStyle}><h3 style={h3Style}>Run Config</h3><div style={{ display: 'grid', gap: 10, marginTop: 12 }}><input value={runConfig[platform].url} onChange={e => setRunConfig(prev => ({ ...prev, [platform]: { ...prev[platform], url: e.target.value } }))} placeholder="URL" style={inputStyle} /><select value={runConfig[platform].resolution} onChange={e => setRunConfig(prev => ({ ...prev, [platform]: { ...prev[platform], resolution: e.target.value } }))} style={inputStyle}>{resolutions.map(size => <option key={size}>{size}</option>)}</select><select value={runConfig[platform].failureMode} onChange={e => setRunConfig(prev => ({ ...prev, [platform]: { ...prev[platform], failureMode: e.target.value } }))} style={inputStyle}><option value="continue">continue</option><option value="stop">stop</option></select><label style={{ fontSize: 12, color: '#334155' }}><input type="checkbox" checked={runConfig[platform].headed} onChange={e => setRunConfig(prev => ({ ...prev, [platform]: { ...prev[platform], headed: e.target.checked } }))} /> Headed</label><div style={{ display: 'flex', gap: 8 }}><button disabled={!selected} style={{ ...btnStyle, background: selected ? '#1d4ed8' : '#94a3b8' }} onClick={() => void runAutoScript(platform)}>Run selected script</button><button style={{ ...btnStyle, background: '#dc2626' }} onClick={() => stopAuto(platform)}>Stop</button></div><div style={{ display: 'flex', gap: 10, fontSize: 12, fontWeight: 700 }}><span style={{ color: '#16a34a' }}>PASS {autoStats[platform].pass}</span><span style={{ color: '#dc2626' }}>FAIL {autoStats[platform].fail}</span><span style={{ color: '#64748b' }}>SKIP {autoStats[platform].skip}</span></div><div style={autoLogStyle}>{autoLogs[platform].length ? autoLogs[platform].map((line, i) => <div key={i}>{line}</div>) : <span style={{ color: '#64748b' }}>Waiting for run...</span>}</div></div></section>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}><section style={panelStyle}><h3 style={h3Style}>Step Progress</h3><div style={{ display: 'grid', gap: 8, marginTop: 12 }}>{steps.length ? steps.map((step, i) => <div key={`${step}-${i}`} style={stepRowStyle}><span>{stepStatuses[platform][i] === 'pass' ? '✅' : stepStatuses[platform][i] === 'fail' ? '❌' : '○'}</span><span>{step}</span></div>) : <span style={emptyStyle}>No script selected</span>}</div></section><section style={panelStyle}><div style={panelHeadStyle}><h3 style={h3Style}>Baseline Management</h3><label style={uploadStyle}>Upload Baseline<input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { void uploadBaseline(platform, e.target.files?.[0]); e.target.value = '' }} /></label></div><div style={thumbGridStyle}>{baselineRows.map(row => <div key={row.id} style={thumbCardStyle}><img src={row.image_path} alt={row.name} style={thumbImageStyle} /><span style={cardTitleStyle}>{row.name}</span><button style={dangerBtnStyle} onClick={() => void deleteBaseline(row.script_id, row.id)}>Delete</button></div>)}</div></section></div>
        {platform === 'pc' && <><section style={panelStyle}><div style={panelHeadStyle}><h3 style={h3Style}>Template Library</h3><label style={uploadStyle}>Upload Template<input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { void uploadTemplate(e.target.files?.[0]); e.target.value = '' }} /></label></div><div style={thumbGridStyle}>{autoTemplates.map(t => <div key={t.id} style={thumbCardStyle}><img src={t.image_path} alt={t.name} style={thumbImageStyle} /><span style={cardTitleStyle}>{t.name}</span><div style={barTrackStyle}><span style={{ ...barFillStyle, width: `${Math.round((t.last_confidence ?? 0) * 100)}%` }} /></div><button style={dangerBtnStyle} onClick={() => { void fetch(`/api/frontend-auto/templates/${t.id}`, { method: 'DELETE' }).then(loadTemplates) }}>Delete</button></div>)}</div></section><section style={panelStyle}><div style={panelHeadStyle}><h3 style={h3Style}>OCR Region Definitions</h3><button style={smallBtnStyle} onClick={() => void addOcrRegion()}>Add Region</button></div><table style={tableStyle}><thead><tr><th style={thStyle}>Name</th><th style={thStyle}>Label</th><th style={thStyle}>Position</th><th style={thStyle}>Accuracy</th><th style={thStyle}>Actions</th></tr></thead><tbody>{ocrRegions.map(r => <tr key={r.id}><td style={tdStyle}>{r.name}</td><td style={tdStyle}>{r.label}</td><td style={tdStyle}>{r.crop_x},{r.crop_y},{r.crop_w},{r.crop_h}</td><td style={tdStyle}>{r.accuracy ?? '-'}</td><td style={tdStyle}><button style={dangerBtnStyle} onClick={() => { void fetch(`/api/frontend-auto/ocr-regions/${r.id}`, { method: 'DELETE' }).then(loadOcr) }}>Delete</button></td></tr>)}</tbody></table></section></>}
      </div>
    )
  }
  // ── 狀態顏色 ────────────────────────────────────────────────────────────────
