...
export function OsmUatPage() {
  ...
  async function createAutoScript(platform: AutoPlatform) {
    ...
    if (!draft.name.trim()) return alert('請填入腳本名稱')
    let parsedSteps: unknown
    try { parsedSteps = JSON.parse(draft.steps) } catch { return alert('步驟格式錯誤：請確認是合法的 JSON 陣列') }
    if (!Array.isArray(parsedSteps)) return alert('步驟格式錯誤：必須是 JSON 陣列 [ ... ]')
    const res = await fetch('/api/frontend-auto/scripts', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: draft.name.trim(), platform, steps: draft.steps, createdBy: actor, isPublic: true }),
    })
    if (!res.ok) {
      const err = await res.json().catch(() => ({ message: '未知錯誤' })) as { message?: string }
      return alert(`儲存失敗：${err.message ?? '未知錯誤'}`)
    }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 3fr) minmax(320px, 2fr)', gap: 16 }}>
          ...
          <section style={panelStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}><label style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>解析度（錄製 / 執行共用）</label><select value={runConfig[platform].resolution} onChange={e => setRunConfig(prev => ({ ...prev, [platform]: { ...prev[platform], resolution: e.target.value } }))} style={inputStyle}>{resolutions.map(size => <option key={size}>{size}</option>)}</select></div>
