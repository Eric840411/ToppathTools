...
export function RecordedScriptBatch({ scripts, selectedIds, onOrder, agentId, running, busy, onBusy, onRun }: {
  ...
  return <>
    ...
    <section className="uat-panel uat-risk-queue">
      ...
      <div className="uat-tc-record-actions"><button className="uat-btn is-primary" disabled={locked || missing || !selectedIds.length} onClick={() => void start(true)}>試跑選取腳本</button><button className="uat-btn is-quiet" disabled={locked || missing || !selectedIds.length} onClick={() => setConfirm(true)}>正式執行選取腳本</button>{busy && <button className="uat-btn is-danger" onClick={() => void stop()}>停止整個佇列</button>}</div>
      <p>試跑不寫入 Lark。一般 TC 判定失敗會繼續下一份；啟動或連線異常會停止後續派工。執行期間請保留此頁，離開後不再派發下一份腳本。</p>
      {confirm && <div className="uat-multi-confirm"><strong>確認依上述順序正式執行 {selectedIds.length} 份腳本？</strong><p>將更新所綁定 TC 的 PASS／FAIL 與附圖。{repeated.size > 0 && `其中 ${repeated.size} 筆 TC 重複綁定，後執行的腳本會覆蓋先前回寫。`}</p><button className="uat-btn is-primary" disabled={locked} onClick={() => void start(false)}>確認執行佇列</button><button className="uat-btn is-quiet" onClick={() => setConfirm(false)}>取消</button></div>}
