...
export function RecordedScriptLibrary({ revision, disabled, onOpen, selectedIds, onSelection, onScripts }: {
  ...
  return <>
    ...
    <div className="uat-tc-record-actions"><button className="uat-btn is-primary" disabled={disabled} onClick={() => onOpen()}>錄製腳本</button><button className="uat-btn is-quiet" disabled={loading} onClick={() => setRefresh(n => n + 1)}>重新整理</button></div>
