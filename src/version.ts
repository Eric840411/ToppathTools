export const APP_VERSION = '4.259.2'
...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  { version: '4.257.0', date: '2026-09-23', changes: ['feat(UAT)：**腳本可以改綁到另一張 Lark TC 表了**（使用者回報：在新表建了 TC，舊腳本沿用不了）。原因是腳本存檔時把 `tableId` 一起存進去、候選清單又只顯示那張表的 TC，而畫面上那行「表格：tblXXX」**是唯讀的沒有入口**。Backend 與 H5／PC 都有', 'feat(UAT)：流程是「貼新網址 → 產生配對建議 → 逐列確認 → 套用」。**絕不自動套用**（CodeX：編號相同不代表同一個測項），只在**編號兩邊都非空且唯一**時給建議，配不上的一律留白', 'fix(UAT)：**換綁定一定要同步搬步驟歸屬。**步驟靠 `step.tcId === binding.recordId` 分配，只換 bindings 會整份驗證不過。而且**沒接上的 `tcId` 要保留原值、不能清成 `null`**——現有的「解除綁定」就是清成 null ＝ 變成共用步驟，畫面只顯示「待指定」，**看不出它原本屬於哪個 TC**', 'feat(UAT)：**先備份再改**，備份含步驟歸屬（只存 bindings 是還原不回來的），可以還原到任何一次改綁之前。新表 `uat_tc_retarget_backups`', 'feat(UAT)：伺服器端**重撈目標表驗一次**，不信前端傳來的 `recordId`——傳錯的話回寫會寫到別張表上而且不報錯。四個端點 `/api/osm-uat/retarget/plan｜apply｜backups｜restore`', 'refactor(UAT)：規則本體放 `shared/uat-tc-retarget.ts`，**前端兩邊與伺服器 import 同一份**；彈窗 `TcRetargetDialog.tsx` 也是 Backend 與 H5／PC 共用', 'test(UAT)：17 條，突變六個逐條驗過都紅在對應那一條', '⚠️ **沒驗到的**：四個端點只確認過路由存在且擋未登入，**沒有對真的 Lark 表跑過完整的 plan → apply → restore**（本機沒有可用的 TC 表）。真環境第一次用要確認候選清單換成新表、步驟歸屬跟著搬、還原退得回去'] },
