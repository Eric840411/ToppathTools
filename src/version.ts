...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  { version: '4.165.0', date: '2026-09-18', changes: ["feat(uat): CDP 連接埠改成讓 Chrome 自己挑——三處原本都是 9300/9400 + random(400)", "⚠️ 亂數的問題不是「機率低」而是「**撞到之後不會失敗，會接到別人的瀏覽器**」：第二顆 Chrome 綁不上那個 port，但  照樣回應（因為第一顆正在那裡聽），於是第二個 session 安靜地操作第一個 session 的視窗——兩邊都跑完、結果交叉污染、沒有任何錯誤訊息", "⚠️ 換更大的亂數範圍只是把機率調小，**歸屬問題完全沒解決**。改用 ，Chrome 挑完寫進 ；每個 session 的 profile 目錄本來就獨立，所以讀到的 port **必然屬於自己開的那顆**，撞號與歸屬一起消失", "⚠️ spawn 前要先清掉舊的 ——profile 目錄重用時會讀到上一顆瀏覽器的 port，那顆可能已關（連不上）或更糟、還活著", "🚨 fix(uat):  的查詢起點改到導頁**之前**（CodeX review P2）。原本設在 goto 完成又等三秒之後，**載入期間打的 API 全部落在界線之前**，斷言會永遠看到 0 支——而「開這頁時打了哪些後端」正是最常要驗的東西。Backend 的 block-engine 早就踩過並在註解裡寫明，我在 H5/PC 兩個引擎上原封不動重現了一次", "🚨 fix(ui-checks): 剝註解的修復接上剩下的地方（CodeX review P2）—— 與  共 6 處還在用舊正則", "⚠️ 接上之後當場抓到一條**原本假通過**的負面斷言：「錄製器不再產出 :nth-match」讀的是  **這個檔案**，而那支整份是 template literal、註解有兩層；舊正則剛好把兩層都抹掉所以是綠的。改成驗**產出的腳本**——比對的是真正的產物，註解也只剩一層", "🚨 fix(ui-checks): 補上 agent 白名單的盲點。被  靜態 import 卻**沒進白名單**的檔案，會從推導集合裡消失—— 看不見它，（只走 spawn 進入點）也看不見。**兩支檢查同時盲掉，而後果是 agent 在 import 當下整支炸掉**。實際發生過： 加了 import 卻忘了白名單，兩支都綠", "test(uat):  44 項（新增三條釘住 netMark 必須在導頁之前，三個引擎一致）； 154 項； 23 項，新增的盲點守門注入後轉紅", "⚠️ **agent 端要按「更新程式碼」再重啟**（ 為新檔）"] },
