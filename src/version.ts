export const APP_VERSION = '4.253.1'
...
export const CHANGELOG: ChangelogEntry[] = [
  { version: '4.253.1', date: '2026-09-22', changes: ['📝 **撞到上限的措辭改成「尚未確認」**（CodeX 補的邊界）：最後一輪剛好關掉最後一個時，畫面其實是乾淨的——我們只是少跑了那一輪確認。原本寫「仍有彈窗」是**講了自己不知道的事**', '🧪 ⑧c 兩組把這條釘住：剛好用完上限（畫面其實乾淨）／真的超過上限（確實有殘留），兩種都要回報，但措辭不能斷定。43 → 52 條', '📝 **更正我自己的說法**（CodeX 指正）：這題不是「只有真環境驗得到」，是**我原本的 fixture 沒涵蓋到**——⑧ 已證明在 fixture 裡重現得出來。真環境的價值在於告訴我「要去擺幾個」，不在於只有它驗得到', '📝 ⑧ 的註解標明是**縮減重現**（4 個 ✕ ＋ 1 個 Confirm），現場的 6 個原樣紀錄在 docs'] },
  { version: '4.253.0', date: '2026-09-22', changes: ['🚨 **使用者給網址實測，打臉我前面的診斷**：Confirm 沒被點掉的真正原因是**輪數上限被前面的種類吃光**，不是（只是）呼叫時機。現場同時有 1 個 `closeBtn` ＋ 4 個 `notification-close` ＋ 1 個 `Tips: Game exception...(39)` 的 Confirm 框——上限是 4，**四輪全花在 ✕ 上，Confirm 那一步一次都沒跑到**', '⚠️ 而且回報寫的是「關掉 4 個彈窗」，**跟正常收工長得一模一樣**，完全看不出還有東西蓋著', '✨ 上限改成**安全網**（預設 4 → 12），真正的結束條件仍是「這一輪什麼都沒關到」；**撞到上限一定 log 並記進 `blocked`**，不再無聲地少做事', '✅ 對同一條網址重跑：6 個全關掉、`(39)` 進了 errors、大廳 688 張卡片都在', '⚠️ **這一題只有真環境看得到**——我前面自製的 fixture 每次只放一兩個彈窗，永遠不會撞到輪數上限，所以 17 條全是綠的。⑧／⑧b 兩條把這個現場釘住了。38 → 43 條', '⚠️ 突變：上限改回 4 → ⑧ 3 條紅（重現真實現場）；撞上限不講話 → ⑧b 1 條紅'] },
  { version: '4.252.4', date: '2026-09-22', changes: ['📝 文件更正（CodeX 指正）：**⑦c 是行為檢查不是結構檢查**——它實際呼叫 `evaluateReadyGate()` 並檢查回傳的失敗原因，只有 ⑦b 才是讀原始碼。我原本把兩者一起歸成結構檢查', '⚠️ 為什麼這個分類要寫對：記成結構檢查，下一個人會以為沒驗過而重做一遍；反過來把結構檢查當成行為檢查更糟——會以為執行路徑已經驗到了'] },
  { version: '4.252.3', date: '2026-09-22', changes: ['🧪 補兩條守衛（CodeX 指出 ⑦b 仍是結構檢查）：① gate 的失敗原因**不能含 `timeout` 字樣**——上層是 `/timeout/i.test(m) ? timeout : err`，寫錯字就會把「畫面沒就緒」標成逾時，查的人會跑去查網路；② `prepare` 丟錯確實會落到 `postStatus(err|timeout)`，不是被吞掉', '⚠️ 失敗原因字串**從產品那支 `evaluateReadyGate()` 取**，不在測試裡再打一次——打一次就變成在驗我自己打的字。34 → 38 條', '❗ **明確留白**：完整的「`prepare` 丟錯 → 任務狀態變 `err`」流程**沒有跑過**。要跑它得做一個假大廳，而假到不像的 fixture 會讓斷言一直綠著——這一格留白等真環境，不用假資料填滿。已寫進 `docs/features/17-ui-screenshot.md`'] },
  { version: '4.252.2', date: '2026-09-22', changes: ['🚨 **[P3] 快速路徑也漏了「關過錯誤框就重驗推流」那道關卡**（CodeX 第三次抓到，同一個 bug 的第三個住址）。主路徑補好了，但「重新載入後已在機台內」那條照樣直接 return——重新載入時推流正常、延遲期間才出錯的話，一樣拍黑畫面而狀態寫 `ok`', '⚠️ **同一條規則寫兩次，就一定會只修好一邊。**所以不是「把那段也補上」，是把決策抽成純函式 `evaluateReadyGate()`，兩條路 import 同一支', '🧪 副作用是**測試終於驗得到「重驗失敗不准報 ok」本身**，不必只做結構檢查——這正是 CodeX 指出 ⑥／⑥b 都還沒驗到的那一格。25 → 34 條', '⚠️ 突變逐條驗過：gate 忽略 `sawErrorPopup` → ⑦ 3 條紅；重驗失敗照樣放行 → ⑦ 2 條紅；快速路徑又不套關卡 → ⑦b 1 條紅'] },
  { version: '4.252.1', date: '2026-09-22', changes: ['🚨 **[P1] 看門狗刻意不點的未知彈窗，被主流程點掉了**（CodeX 靜態核對抓到）。主流程在看門狗運作中**另外**呼叫了一次關窗、而且沒帶 `strict`——strict 等於白設，而且兩條路會同時點同一顆按鈕', '⚠️ **修法不是「記得不要那樣呼叫」，是讓它做不到**：`dismissUiPopups()` 自己查 `ACTIVE_GUARDS`，這個 page 上有看門狗在跑就一律導進它的佇列並降級成 strict。靠紀律維持的不變量，遲早會有人（包括我）在別的地方再寫一次', '🚨 **[P2]「先就緒、延遲期間才出錯」不會重驗推流**。重驗條件原本只寫 `if (!ready)`——`ready` 已經是 true、錯誤框在等截圖那幾秒才跳出來被關掉，就直接往下拍，**拍到黑畫面而狀態欄寫 `ok`**。改成記下 `sawErrorPopup`，關過錯誤框就一律重驗', '🔧 「重新載入後已在機台內」那條快速路徑底下的等待也補上看門狗——同一個坑，只是少有人走到', '🧪 驗證 17 → 25 條，補上 CodeX 指定的兩個整合情境。⑤ 驗的是**用最容易犯錯的寫法去呼叫也點不掉**，不是「檢查沒人那樣寫」', '⚠️ 三條缺陷分開注入，紅的分別是 ⑤ 3 條、⑥b 1 條、⑥b 結構守衛 1 條'] },
  { version: '4.252.0', date: '2026-09-22', changes: ['🚨 **UI 截圖：進機台跳 Confirm 彈窗沒被點掉，流程卡住**（使用者回報）。查下去發現關彈窗的功能一直都在，**缺的是呼叫時機**——只在「進機台前」「推流就緒後」「大廳模式」三個點各跑一次，中間三段沒人看：點卡片那一下、等推流的迴圈、截圖前等的那幾秒', '⚠️ 這三段冒出彈窗的症狀分別是「點了但還停在大廳」「Game surface not ready」「拍到被蓋住的畫面」——**三種訊息都不會提到彈窗**，所以一直被當成別的問題在查', '✨ **進機台 → 等推流 → 截圖前延遲，整段掛看門狗**，每 0.7 秒看一次；序列化、有次數上限，截圖前一定先 stop 並等它跑完（否則會拍到「正在被點掉」的瞬間）', '🚨 **看門狗只點「已確認用途」的彈窗**（CodeX 建議）：面額選單、Tips 錯誤框、純提示框。沒見過的**不點**，把內容跟著那張截圖一起回報。理由：一次性關一下跟每 0.7 秒關一次，風險不是同一件事——連續點 Confirm 遲早會在遊戲畫面裡點到不該點的，而症狀是「流程從沒人預期的狀態繼續跑」，比卡住更難查', '🐛 **`ensureUiScreenshotLobby()` 原本無條件關彈窗**，繞過了畫面上的「自動關閉面額彈窗」開關（CodeX 指出）。使用者關掉開關是想看彈窗長什麼樣，結果這條路照樣幫他關', '⚠️ **等待期間關掉錯誤框之後要重新確認推流**才算數——直接往下走會拍到黑畫面而狀態寫 `ok`', '🔧 彈窗邏輯搬到 `server/uat-runner/ui-popup.js`（已加進 `AGENT_SOURCE_WHITELIST`）。原本寫在 `agent-runner.ts` 裡驗證腳本碰不到，要驗只能再抄一份——那等於測試在驗自己', '🧪 `scripts/ui-checks/ui-popup-dismiss.mjs` 17 條：延遲出現／連續兩層／未知 Confirm 不點／設定生效，加上「裸 Confirm 字樣不點」「中獎彈窗不得點到 PLAY NOW」兩個反例。突變逐條驗過，紅的都是對應的那幾條'] },
